...
int main (int argc, char *argv[])
{
  ...
  // example below: "AfterElectronChargeProduct" and "AfterTau_decayModeFinding".
  ...
  theDirectory[0] = OutputHistos->mkdir("AfterElectronChargeProduct");
                  ...
                  for (int t = 0; t < Tau_pt->size(); t++)
                    {
                     ...
                     if ( Tau_pt->size() != (t+1)){
                       if (DeltaR_electron_tau < DeltaR_matched) {
                          DeltaR_matched  = DeltaR_electron_tau;
                          matched_tau_index = t;
                       } 
                     }else {
                      if (DeltaR_electron_tau < DeltaR_matched) {
